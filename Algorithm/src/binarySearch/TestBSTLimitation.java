...
/**
 * 对于十分有序的数据，二分搜索树会退化为链表
 * 对于这种情况，其优化就是平衡二叉树的概念，平衡二叉树的最著名的实现就是红黑树
 * @author Administrator
 *
 */
public class TestBSTLimitation {
    ...
    public static void main(String[] args) {
        ...
        if(FileOperations.readFile(filename, words)){
            ...
            // 发现对于十分有序的数据，二分搜索树的效率比链表还低
            // 这是因为对于十分有序的数据，二分搜索树会退化为链表，并且由于还要判断
            // 左孩子并且实现方法是递归，所以效率比单链表还低
            startTime = System.currentTimeMillis();
            BinarySearchTree<String, Integer> bst2 = new BinarySearchTree<String, Integer>();
            Collections.sort(words); // 对words进行排序
            for (String word: words) {
